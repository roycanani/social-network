...
export function ChatRow({ chat }: ChatRowProps) {
  ...
  const initials = currentUser?.user?.userName!
    .split(" ")
    .map((n) => n[0])
    .join("")
    .toUpperCase()
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-baseline mb-1">
          <h3 className={cn("text-sm font-medium truncate", unread && "font-semibold")}>{otherUser?.userName}</h3>
          <span className="text-xs text-muted-foreground whitespace-nowrap ml-2">{timeAgo}</span>
        </div>
        <p className={cn("text-xs text-muted-foreground truncate", unread && "text-foreground font-medium")}>
          {lastMessageContent}
        </p>
      </div>
